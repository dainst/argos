defmodule ArgosCore.ThesauriTest do
  use ExUnit.Case

  require Logger

  doctest ArgosCore.Thesauri
  ...
  describe "dataprovider tests" do
    ...
    test "get by id with invalid id yields 404" do
      {:error, %{status: 404}} = DataProvider.get_by_id("i-am-non-existant")
    end
  end
  ...
  describe "elastic search integration tests" do
    ...
    test "concept can be added to index" do
      ...
      indexing_response = ArgosCore.ElasticSearch.Indexer.index(concept)

      %{
        upsert_response: {:ok, %{ "_id" => "concept__b7707545", "result" => "created"}
      }} = indexing_response
    end
    ...
    test "concept can be reloaded locally" do
      ...
      assert length(concept_from_index.core_fields.title) - 1 ==
               length(concept_from_thesaurus.core_fields.title)
    end
    ...
    test "if concept was requested to be loaded locally, but was missing in the index, it is also automatically indexed" do
      ...
      assert {:ok, _concept_from_index} =
               ArgosCore.ElasticSearch.DataProvider.get_doc(concept.core_fields.id)
    end
  end
  ...
  describe "concept parser tests" do
    ...
    test "parse search result" do
      with {:ok, body} <- File.read("test/data_sources/xml_test_files/test_valid_search_rdf.xml") do
        ...
        assert 'http://thesauri.dainst.org/search.rdf?change_note_date_from=2021-01-01&page=1&q=' ==
                 url

        {doc_list, next_url} = ConceptParser.Search.load_next_page_items(body)

        assert 'http://thesauri.dainst.org/search.rdf?change_note_date_from=2021-01-01&page=2&q=' ==
                 next_url
      ...
      with {:ok, body} <- File.read("test/data_sources/xml_test_files/invalid_documents.rdf") do
        ...
        assert %{
                 "core_fields" => %{
                   ...
                   "title" => [
                     %{"lang" => "de", "text" => ""},
                     %{"lang" => "en", "text" => "iDAI.world thesaurus"}
                   ]
                 }
               } = doc
        ...
        assert %{
                 "core_fields" => %{
                   ...
                   "title" => [
                     ...
                     %{"lang" => "en", "text" => "gods and mythological figurs"}
                   ]
                 }
               } = doc
      else
    ...
    test "parse hierarchy result" do
      with {:ok, body} <- File.read("test/data_sources/xml_test_files/valid_hierarchy.rdf") do
        ...
        assert %{
                 "core_fields" => %{
                   "source_id" => "_b189d13f",
                   "title" => [%{"lang" => "de", "text" => "Afrikanische Archäologie"}]
                 }
               } = concept
      else
        {:error, error} -> raise error
  ...
  describe "elastic search tests" do
    ...
    test "concept can be reloaded locally" do
      ...
      # Finally compare the title field length.
      assert length(concept_from_index.core_fields.title) - 1 ==
               length(concept_from_thesauri.core_fields.title)
    end
